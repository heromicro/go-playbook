...
var Tmplt_ansible_builtin_dnf = `
{{ Indent " " 4}}- name: dnf operation
...
type AnsibleBuiltinDnf struct {
	...
	Nobest           enumtipe.CostomBool       `json:"nobest"`
...
func (a *AnsibleBuiltinDnf) MakeAnsibleTask() (string, error) {
	...
	tmpl := template.Must(template.New("ansible_builtin_dnf").Funcs(funcMap).Parse(Tmplt_ansible_builtin_dnf))
